...
* Install using HACS, or manually: copy all files in custom_components/cover_time_based_sync to your <config directory>/custom_components/cover_time_based_sync/ directory.
* Restart Home-Assistant.
* Create the required scripts in scripts.yaml.
* Add your new devide to HA in ´Configuration / Devices & Services / Add New / Cover Time Based Sync`
...
- `always_confident` defaults to `False`. If set to `True`, the calculated state will always be assumed to be accurate. This mainly affects UI components - for example, if the cover is fully opened, the open button will be disabled. Make sure to [set](#cover_time_based_syn.set_known_position) the current state after first setup and only use this entity to control the covers. Not recommended to be `True` for RF-based covers.
